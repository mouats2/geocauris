...
function Dashboard({ user }: { user: User }) {
  ...
  function buy() {
    setNotice("La recharge en ligne est temporairement indisponible : aucun moyen de paiement n'est configuré pour le moment. Contactez le support pour recharger votre compte.");
  }
...
function Pack({ name, credits, priceXof, featured, onBuy }: { name: string; credits: number; priceXof: number; featured?: boolean; onBuy: () => void }) { const price = `${priceXof.toLocaleString("fr-FR")} FCFA`; return <article className={`pack-card ${featured ? "featured" : ""}`} role="button" tabIndex={0} aria-label={`Acheter ${credits.toLocaleString("fr-FR")} cauris pour ${price}`} onClick={onBuy} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onBuy(); } }}><div className="pack-card-top"><span>{name}</span>{featured && <b>Le plus choisi</b>}</div><strong>{credits.toLocaleString("fr-FR")}</strong><small>cauris</small><div className="pack-price">{price}</div><button onClick={(event) => { event.stopPropagation(); onBuy(); }}>Choisir <ArrowUpRight size={16} /></button></article>; }
