...
function Dashboard({ user }: { user: User }) {
  ...
  async function buy(credits: number, price: string) {
    ...
    try {
      ...
      const response = await fetch("/api/payments/saspay", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ packId: pack.id }) });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) { setNotice(data.error ?? `Impossible de créer le paiement de ${price}.`); return; }
      if (!data.paymentUrl) { setNotice("Le paiement a été créé mais SasPay n'a pas fourni de lien de paiement."); return; }
      window.location.assign(data.paymentUrl);
    } catch { setNotice("Impossible de contacter SasPay. Vérifiez votre connexion puis réessayez."); } finally { setBuyingPack(null); }
  }
