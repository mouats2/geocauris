...
function Dashboard({ user }: { user: User }) {
  ...
  async function buy(credits: number, price: string) {
    ...
    try {
      ...
      const response = await fetch("/api/payments/fedapay", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ packId: pack.id }) });
