...
export async function POST(request: Request) {
  ...
  try { authenticatedUser = await requireUser(request); } catch { return NextResponse.json({ error: "Authentification requise" }, { status: 401 }); }
  ...
  const upstream = await fetch(`${apiBase.replace(/\/$/, "")}/transactions`, {
    ...
    body: JSON.stringify({
      ...
      customer: { email },
      custom_metadata: { packId: pack.id, credits: pack.credits, uid: authenticatedUser.uid },
    }),
    cache: "no-store",
  });
